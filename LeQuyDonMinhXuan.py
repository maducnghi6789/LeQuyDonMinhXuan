...
# ==========================================
# 1. TIỆN ÍCH XỬ LÝ USERNAME & TOÁN HỌC
# ==========================================
...
def clean_ai_json(json_str):
    """Làm sạch JSON dùng mã hóa ký tự chr(96) để chống lỗi SyntaxError"""
    res = json_str.strip()
    bt = chr(96) 
...
def format_math(text):
    """Sửa lỗi hiển thị công thức Toán LaTeX"""
...
# ==========================================
# 3. ĐỘNG CƠ AI (VƯỢT RÀO 404/429 & HYBRID MATRIX)
# ==========================================
...
def safe_ai_generate(prompt, api_key):
    """Cơ chế Vượt rào API của bản A1"""
...
def parse_exam_with_ai(raw_text, api_key):
    prompt = f"Biên tập PDF thành JSON 40 câu Toán. Chuẩn LaTeX bọc trong $ hoặc $$. Escape backslash. KHÔNG dùng backtick. DỮ LIỆU: {raw_text}"
...
def generate_free_practice_hybrid(api_key):
    ...
    if num_ai > 0:
        ...
        prompt = f"Sáng tác {num_ai} câu Toán 9 VDC. Tránh trùng nội dung với: {ctx}. Trả về JSON mảng câu hỏi, dùng chuẩn LaTeX bọc trong $ hoặc $$. Escape backslash."
...
# ==========================================
# 4. GIAO DIỆN LÀM BÀI & XEM LẠI (REVIEW MODE)
# ==========================================
def take_exam_ui(exam_data, exam_id, is_mandatory=True, is_review=False, user_ans_data=None):
    ...
    if not st.session_state.show_results:
        timer_html = f"<div style='position:fixed;top:60px;right:20px;background:#ff4b4b;color:white;padding:10px;border-radius:8px;z-index:9999;'>⏳ CÒN LẠI: <span id='timer'>90:00</span></div>"
        st.components.v1.html(timer_html + "<script>var limit=90*60*1000;var start=new Date().getTime();var x=setInterval(function(){var now=new Date().getTime();var distance=limit-(now-start);var m=Math.floor((distance%(1000*60*60))/(1000*60));var s=Math.floor((distance%(1000*60))/1000);document.getElementById('timer').innerHTML=m+'p '+s+'s ';if(distance<0){clearInterval(x);window.parent.document.querySelectorAll('button').forEach(b=>{if(b.innerText.includes('NỘP BÀI'))b.click();});}},1000);</script>", height=0)
        ...
        st.info("Nộp bài thành công! Hãy nhấn vào tên bài trong danh sách để xem chi tiết.")
        if st.button("Quay lại"): st.session_state.show_results = False; st.session_state.active_exam = None; st.rerun()

# ==========================================
# 5. GIAO DIỆN ĐIỀU HƯỚNG CHÍNH A1
# ==========================================
def main():
    st.set_page_config(page_title=f"LMS {VERSION}", layout="wide")
    init_db()
    if 'current_user' not in st.session_state:
        col1, col2, col3 = st.columns([1,1.2,1])
        with col2:
            st.title(f"🎓 ĐĂNG NHẬP")
            with st.form("login"):
                u = st.text_input("User").strip(); p = st.text_input("Pass", type="password").strip()
        ...
        with st.sidebar:
            ...
            if role == "core_admin":
                ...
                if st.button("Lưu API"):
                    conn = sqlite3.connect('exam_db.sqlite'); conn.execute("INSERT OR REPLACE INTO system_settings VALUES ('GEMINI_API_KEY', ?)", (new_key,)); conn.commit(); st.success("Đã lưu API"); st.rerun()
        ...
        if choice == "✍️ Kiểm tra bắt buộc":
            ...
            for eid, title, qj in exams:
                ...
                # Nâng cấp 1: Biến Test1 thành nút bấm xem lại bài làm
                if done:
            ...
            st.header("🚀 Luyện đề tự do") # Nâng cấp 3: Bỏ chữ mô tả rườm rà
            if st.button("🪄 TẠO ĐỀ HYBRID (APP + AI)"):
                with st.spinner("AI đang phối hợp tạo đề..."):
                    res = generate_free_practice_hybrid(api_key)
                    if isinstance(res, list): st.session_state.free_exam = {'title': 'Đề luyện tập A1', 'questions':res}
                    else: st.error(res)
            ...
            if not api_key: st.error("Chưa có API Key!")
                ...
                if st.session_state.get('temp_exam'):
                    ...
                    if st.button("💾 XÁC NHẬN GIAO ĐỀ"):
                        conn = sqlite3.connect('exam_db.sqlite'); conn.execute("INSERT INTO mandatory_exams (title, questions_json, target_class, created_by) VALUES (?,?,?,?)", (data['t'], json.dumps(data['q']), data['c'], st.session_state.current_user)); conn.commit(); st.success("Xong!"); del st.session_state.temp_exam; st.rerun()
