...
def init_db():
    ...
    # --- ĐỘNG CƠ TỰ NÂNG CẤP (MIGRATION) ---
    # Kiểm tra và thêm cột 'school' nếu thiếu
    try: c.execute("ALTER TABLE users ADD COLUMN school TEXT")
    except: pass
    # Kiểm tra và thêm cột 'managed_classes' nếu thiếu
...
# ==========================================
# 3. MODULE QUẢN LÝ TÀI KHOẢN ĐỒNG BỘ (FIXED KEYERROR)
# ==========================================
def account_manager_ui(target_role, specific_class=None):
    ...
    if not df.empty:
        # Đảm bảo các cột hiển thị tồn tại trong DataFrame (Chống lỗi KeyError)
        cols_to_show = ['username', 'fullname', 'password', 'class_name']
        if 'school' in df.columns: cols_to_show.append('school')
        if 'managed_classes' in df.columns: cols_to_show.append('managed_classes')
        ...
        if sel_u != "-- Chọn --":
            ...
            with st.form(f"form_{sel_u}"):
                ...
                f_man = st.text_input("Quyền quản lý (Dành cho Admin thành viên)", value=u_data.get('managed_classes', '') if pd.notna(u_data.get('managed_classes')) else "")
...
def import_student_module():
    ...
    t1, t2 = st.tabs(["📁 File Excel", "✍️ Nhập tay"])
    with t1:
    ...
    if 'current_user' not in st.session_state:
        ...
        if choice == "🛡️ Quản trị tối cao":
            ...
            with t2:
                ...
                conn.close()
                selected_class = st.selectbox("📌 Chọn lớp học giám sát:", ["Tất cả các lớp"] + all_classes)
            ...
            t1, t2 = st.tabs(["📥 Nhập dữ liệu học sinh", "🛠️ Tác vụ Sửa/Xóa"])
            with t1: import_student_module()
            with t2: account_manager_ui("student", specific_class=st.session_state.managed)
