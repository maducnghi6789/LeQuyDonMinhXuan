...
# --- CẤU HÌNH HỆ THỐNG V42 (A1 SUPREME - TỐI GIẢN UI TUYỆT ĐỐI) ---
...
def generate_ai_exam_for_admin(api_key):
    ...
    YÊU CẦU ĐỊNH DẠNG (JSON): [{"q": "...", "options": ["A.", "B.", "C.", "D."], "ans": "A", "exp": "Giải nhanh..."}]
    Bọc công thức Toán trong dấu $. Dùng nháy đơn (') bên trong chuỗi, KHÔNG dùng nháy kép ("). CHỈ TRẢ VỀ JSON.
    """
...
def parse_admin_exam_with_ai(raw_text, api_key):
    ...
    YÊU CẦU ĐỊNH DẠNG (JSON): [{{"q": "...", "options": ["A.", "B.", "C.", "D."], "ans": "A", "exp": "Giải nhanh..."}}]
    Bọc các công thức trong dấu $. Dùng nháy đơn (') bên trong chuỗi. CHỈ TRẢ VỀ JSON.
    ...
    if 'current_user' not in st.session_state:
        ...
        if choice == "🛡️ Quản trị chung":
            ...
            if not api_key: st.error("❌ Hệ thống chưa cấu hình Gemini API Key.")
                ...
                t_ai, t_admin = st.tabs(["🤖 AI Tự Sinh", "📝 AI Giải Đề Soạn Sẵn"])
                
                with t_ai:
                    with st.form("form_ai_gen"):
                        e_title_ai = st.text_input("Tên bài kiểm tra:", value="Đề Kiểm Tra Toán")
                        ...
                        if st.form_submit_button("🚀 TẠO VÀ GIAO ĐỀ"):
                            if e_title_ai:
                ...
                with t_admin:
                    with st.form("form_admin_manual"):
                        e_title_man = st.text_input("Tên bài kiểm tra:", value="Đề Kiểm Tra Toán")
                        ...
                        e_file = st.file_uploader("File PDF", type="pdf")
                        
                        if st.form_submit_button("🚀 XỬ LÝ & GIAO ĐỀ"):
                            if e_title_man and (e_text.strip() or e_file):
                                with st.spinner("Đang xử lý..."):
                                    ...
                                    if isinstance(exam_res, list):
                                        ...
                                        conn.commit(); conn.close()
                                        st.success(f"✅ Đã giao {len(exam_res)} câu hỏi cho lớp {e_class_man}!")
                                    else: st.error(f"❌ {exam_res}")
