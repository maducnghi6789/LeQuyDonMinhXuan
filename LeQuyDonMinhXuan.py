...
# --- CẤU HÌNH HỆ THỐNG V39 (A1 SUPREME - TỐI GIẢN MENU & FULL ĐỒ THỊ/BIỂU ĐỒ SVG) ---
...
# Bổ sung Biểu đồ cột, Biểu đồ quạt, Đồ thị Parabol, Đường tròn giao điểm
# ==========================================
...
def svg_parabola_intersection():
    ...
    <svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="105" y="155" font-family="Arial" font-size="12" font-weight="bold">O</text>
        ...
        <text x="40" y="125" font-family="Arial" font-size="13" font-weight="bold" fill="#0f172a">A</text>
        <circle cx="140" cy="60" r="4" fill="#0f172a"/>
        <text x="145" y="65" font-family="Arial" font-size="13" font-weight="bold" fill="#0f172a">B</text>
    </svg></div>
    """
...
def generate_algorithmic_practice():
    ...
    # --- CHƯƠNG 2: HÀM SỐ & ĐỒ THỊ (Có Đồ thị Parabol SVG) ---
    a7 = random.choice([-3, -2, 2, 3])
    x7 = random.randint(1, 3)
    q7_opts, q7_ans = make_opts(a7*(x7**2), -a7*(x7**2), a7*x7, -a7*x7)
    questions.append({
        "q": f"Biết điểm $M({x7}; y_0)$ thuộc đồ thị hàm số $y = {a7}x^2$. Giá trị của $y_0$ là:",
        "options": q7_opts, "ans": q7_ans, "exp": f"Thay $x = {x7}$ vào hàm số: $y_0 = {a7} \cdot ({x7})^2 = {a7*(x7**2)}$."
    })

    q9_opts, q9_ans = make_opts("A(-1; 1) \\text{ và } B(2; 4)", "A(1; -1) \\text{ và } B(4; 2)", "A(-1; -1) \\text{ và } B(2; -4)", "A(1; 1) \\text{ và } B(2; 2)")
    questions.append({
        "q": f"Dựa vào đồ thị dưới đây, tọa độ các giao điểm của parabol $(P): y = x^2$ và đường thẳng $(d): y = x + 2$ là:",
        "svg": svg_parabola_intersection(),
        "options": q9_opts, "ans": q9_ans,
        "exp": "Quan sát trên đồ thị, hai điểm cắt nhau rõ ràng tại $A$ có tọa độ $x=-1, y=1$ và điểm $B$ có tọa độ $x=2, y=4$."
    })

    m10 = random.randint(2, 5)
    q10_opts, q10_ans = make_opts(m10-2, m10+2, 2-m10, 0)
    questions.append({
    ...
    q12_opts, q12_ans = make_opts("m = \\pm 1", "m = 1", "m = -1", "m = 2")
    questions.append({
    ...
    q13_opts, q13_ans = make_opts("(3; 2)", "(2; 3)", "(1; -2)", "(-3; -2)")
    questions.append({
    ...
    q14_opts, q14_ans = make_opts(f"\\{{1; {c14}\\}}", f"\\{{-1; -{c14}\\}}", f"\\{{0; {c14}\\}}", f"\\{{1; -{c14}\\}}")
    questions.append({
    ...
    q15_opts, q15_ans = make_opts(S15, -S15, P15, -P15)
    questions.append({
    ...
    q17_opts, q17_ans = make_opts("4", "2", "0", "1")
    questions.append({
    ...
    q19_opts, q19_ans = make_opts("7", "9", "11", "5")
    questions.append({
    ...
    q21_opts, q21_ans = make_opts("x < 4", "x > 4", "x \\ge 4", "x \\le 4")
    questions.append({
    ...
    q23_opts, q23_ans = make_opts("m > \\frac{5}{2}", "m < \\frac{5}{2}", "m \\ge \\frac{5}{2}", "m \\neq \\frac{5}{2}")
    questions.append({
    ...
    # --- CHƯƠNG 5: HỆ THỨC LƯỢNG (CÓ SVG TÒA NHÀ & CÁI THANG) ---
    b27 = random.randint(4, 15); g27 = random.choice([30, 45, 60]); h27 = round(b27 * math.tan(math.radians(g27)), 1)
    q27_opts, q27_ans = make_opts(f"{h27}m", f"{round(b27/math.tan(math.radians(g27)),1)}m", f"{round(b27*math.sin(math.radians(g27)),1)}m", f"{round(b27*math.cos(math.radians(g27)),1)}m")
    questions.append({
    ...
    q28_opts, q28_ans = make_opts("60^\circ", "30^\circ", "45^\circ", "75^\circ")
    questions.append({
    ...
    # --- CHƯƠNG 6: ĐƯỜNG TRÒN (CÓ SVG ĐƯỜNG TRÒN NỘI TIẾP) ---
    g_noi_tiep = random.choice([30, 45, 60])
    q_circle_opts, q_circle_ans = make_opts(f"{g_noi_tiep*2}^\circ", f"{g_noi_tiep}^\circ", f"{180-g_noi_tiep}^\circ", f"{90-g_noi_tiep}^\circ")
    questions.append({
        ...
        "svg": svg_circle_inscribed_angle(angle=g_noi_tiep),
        "options": q_circle_opts, "ans": q_circle_ans,
        "exp": "Định lý: Số đo góc ở tâm luôn gấp đôi số đo góc nội tiếp cùng chắn một cung."
    })
    
    q32_opts, q32_ans = make_opts("95^\circ", "85^\circ", "105^\circ", "15^\circ")
    questions.append({
    ...
    # --- CHƯƠNG 7: HÌNH KHỐI (CÓ SVG HÌNH TRỤ) ---
    r36 = random.randint(2, 4); h36 = random.randint(5, 8)
    q36_opts, q36_ans = make_opts(f"{2*r36*h36}\pi", f"{r36*h36}\pi", f"{r36**2 * h36}\pi", f"{4*r36*h36}\pi")
    questions.append({
        "q": f"Dựa vào kích thước trên hình vẽ, diện tích xung quanh của khối trụ (cylinder) này là:",
        "svg": svg_cylinder(r_val=str(r36), h_val=str(h36)),
        "options": q36_opts, "ans": q36_ans,
        "exp": f"$S_{{xq}} = 2\pi r h = 2\pi({r36})({h36}) = {2*r36*h36}\pi$."
    })

    # --- CHƯƠNG 8: THỐNG KÊ XÁC SUẤT (CÓ BIỂU ĐỒ CỘT & BIỂU ĐỒ QUẠT SVG) ---
    v1 = random.randint(30, 50); v2 = random.randint(20, 40); v3 = random.randint(35, 45)
    tot_bar = v1 + v2 + v3
    q_bar_opts, q_bar_ans = make_opts(str(tot_bar), str(v1+v2), str(tot_bar+10), str(tot_bar-5))
    questions.append({
        "q": f"Dựa vào biểu đồ cột thể hiện số học sinh giỏi của 3 lớp, tổng số học sinh giỏi của cả 3 lớp khối 9 là bao nhiêu?",
        "svg": svg_bar_chart("9A", v1, "9B", v2, "9C", v3, "SỐ HỌC SINH GIỎI"),
        "options": q_bar_opts, "ans": q_bar_ans,
        "exp": f"Nhìn vào biểu đồ ta cộng 3 cột lại: ${v1} + {v2} + {v3} = {tot_bar}$ học sinh."
    })
    
    p1, p2, p3 = 25, 25, 50
    q_pie_opts, q_pie_ans = make_opts("100", "50", "25", "150")
    questions.append({
        "q": f"Biểu đồ quạt tròn biểu diễn tỉ lệ % học sinh yêu thích các môn học. Biết tổng số học sinh được khảo sát là $200$ em. Số học sinh yêu thích môn Hóa học (phần màu xanh lá, chiếm {p3}%) là bao nhiêu em?",
        "svg": svg_pie_chart(p1, p2, p3),
        "options": q_pie_opts, "ans": q_pie_ans,
        "exp": f"Số học sinh = $200 \\times {p3}\% = 200 \\times 0.5 = 100$ em."
    })

    # --- CÁC CÂU BỔ SUNG ĐỂ ĐỦ 40 CÂU (Lấy từ bộ pool đa dạng ngẫu nhiên) ---
    while len(questions) < 40:
        c_nguyen = random.randint(1, 5)
        ans20 = "4" if c_nguyen+2 in [3,5,7] else "6"
        q20_opts, q20_ans = make_opts(ans20, "2", "8", "Vô số")
        questions.append({
            "q": f"Số cặp số nguyên $(x; y)$ thỏa mãn phương trình $x y - 2x - y = {c_nguyen}$ là:",
            "options": q20_opts, "ans": q20_ans,
            "exp": f"Biến đổi pt thành $x(y-2) - (y-2) = {c_nguyen+2} \Leftrightarrow (x-1)(y-2) = {c_nguyen+2}$. Dựa vào số ước nguyên để tìm số cặp."
        })
    ...
    if is_review and user_ans_data:
        ...
        for i, q in enumerate(questions):
            ...
            with st.expander(f"Câu {i+1}: {icon} | Đáp án chuẩn: {q['ans']}"):
                ...
                # HIỂN THỊ HƯỚNG DẪN THÔNG MINH CHO CÂU SAI
                if not is_correct: 
...
def main():
    ...
    if 'current_user' not in st.session_state:
        ...
        with st.sidebar:
            ...
            # CẬP NHẬT MENU GỌN GÀNG THEO YÊU CẦU
            if role in ["core_admin", "sub_admin"]:
                menu = ["📤 Giao đề thi thử", "📊 Thống kê"]
